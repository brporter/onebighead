...
func (t *testingHTTPClient) Get(URL string) ([]byte, error) {
	...
	if URL == "https://good/oidc" {
		return GoodOidcDocumentWithBadJwksURI, nil
	}

	if URL == "https://bad/oidc" {
		return BadOidcDocument, nil
	}

	if URL == "https://bad/jwks" {
		return BadJwksDocument, nil
	}

	if URL == "https://gone/" {
		return GoneDocument, nil
	}
...
func TestNewAuthenticationMiddleware_BadAuthFile(t *testing.T) {
	testIOHelper.Error = fmt.Errorf("error")
	testIOHelper.FileData = nil

	am, err := middleware.NewAuthenticationMiddleware()

	if am != nil {
		t.Fatalf("Middleware was returned and should not have been.")
	}

	if err == nil || err.Error() != "While initializing, an error was encountered reading the needed configuration data: Unable to read authentication configuration data: error" {
		t.Fatalf("Expected failure not returned.")
	}
}

func TestNewAuthenticationMiddleware_BadConfigURI(t *testing.T) {
	testIOHelper.Error = nil
	testIOHelper.FileData = []byte(`[
		{
			"provider": "foo",
	...
	if err == nil || !strings.HasPrefix(err.Error(), "While initializing, an error was encountered reading the needed configuration data: Failed to refresh auth configuration. Will retry in 0s. Error: Failed to deserialize the JSON retrieved from the configuration document for foo provider, retrieved from url https://bad/oidc") {
		t.Fatalf("Expected failure not returned.")
	}
}

func TestNewAuthenticationMiddleware_GoodConfigURIBadJWKS(t *testing.T) {
	testIOHelper.Error = nil
	testIOHelper.FileData = []byte(`[
		{
			"provider": "foo",
	...
	if err == nil || !strings.HasPrefix(err.Error(), "While initializing, an error was encountered reading the needed configuration data: Failed to refresh auth configuration. Will retry in 0s. Error: Failed to initialize signing keys for provider 'foo': Failed to decode the JWKS document retrieved from https://bad/jwks") {
		t.Fatalf("Expected failure not returned.")
	}
}
...
var GoodOidcDocumentWithBadJwksURI []byte
var BadOidcDocument []byte
...
func init() {
	...
	GoodOidcDocumentWithBadJwksURI = []byte(`{
		"token_endpoint": "https://foo/common/oauth2/v2.0/token",
		"token_endpoint_auth_methods_supported": [
			"client_secret_post",
			"private_key_jwt",
