...
// We need to test the class, so we'll create fresh instances
class ApiClient {
  private config: { baseUrl: string; defaultHeaders?: HeadersInit; onUnauthorized?: () => void };
  private abortControllers: Map<string, AbortController> = new Map();

  constructor(config: { baseUrl: string; defaultHeaders?: HeadersInit; onUnauthorized?: () => void }) {
    this.config = config;
  }
  ...
  async request<T>(
    endpoint: string,
    options: RequestInit & { skipJsonParse?: boolean; timeout?: number } = {},
    requestKey?: string
  ): Promise<T> {
  ...
  get<T>(endpoint: string, options?: RequestInit & { skipJsonParse?: boolean; timeout?: number }, requestKey?: string): Promise<T> {
    return this.request<T>(endpoint, { ...options, method: 'GET' }, requestKey);
  }

  post<T>(endpoint: string, body?: unknown, options?: RequestInit & { skipJsonParse?: boolean; timeout?: number }): Promise<T> {
  ...
  put<T>(endpoint: string, body?: unknown, options?: RequestInit & { skipJsonParse?: boolean; timeout?: number }): Promise<T> {
    return this.request<T>(endpoint, {
      ...options,
      method: 'PUT',
      body: body ? JSON.stringify(body) : undefined,
  ...
  delete<T>(endpoint: string, options?: RequestInit & { skipJsonParse?: boolean; timeout?: number }): Promise<T> {
    return this.request<T>(endpoint, { ...options, method: 'DELETE' });
  }

  upload<T>(endpoint: string, formData: FormData, options?: RequestInit & { skipJsonParse?: boolean; timeout?: number }): Promise<T> {
    return this.request<T>(endpoint, {
      ...options,
      method: 'POST',
      body: formData,
  ...
  download(endpoint: string, options?: RequestInit & { skipJsonParse?: boolean; timeout?: number }): Promise<Response> {
